...
public class StudentListPanel extends JPanel implements FocusListener {
    ...
    // 刷新文件数据
    private JButton refreshFileDataBut = new JButton("刷新文件数据");
    // 修改学生
    ...
    private void init() {
        ...
        // 刷新文件数据到表格
        refreshFileDataBut.setFont(butFont);
        this.add(refreshFileDataBut, BorderLayout.CENTER);
        refreshFileDataBut.addActionListener(e -> {
            StudentData.refreshFileData();
            menuFrame.setVisible(false);
            MainMenuFrame newMain = new MainMenuFrame(-1);
            StudentListPanel newPanel = new StudentListPanel(newMain);
            newPanel.setVisible(true);

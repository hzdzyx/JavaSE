...
public class StudentController {
    ...
    /**
     * 从TCP网络传输,初始化数据源数组
     * 该方法返回0表示,接收数据成功,并正常完成数据源数组的初始化
     * 返回-1表示,整个网络传输过程中发生了异常
     * 返回-2,表示主动关闭停止网络连接
     *
     * @return int
     * @author wuguidong@cskaoyan.onaliyun.com
    ...
    public int initDataFromNet() {
        return studentService.initDataFromNet();
    }

    /**
     * 关闭用于初始化数据源数组的TCP网络资源
     * 注意:
     * 此方法关闭的TCP资源和initDataFromNet方法使用的是同一资源
     *
     * @author wuguidong@cskaoyan.onaliyun.com
     * @since 11:25
     */
    public void closeTCPSocket(){
        studentService.closeTCPSocket();
    }


    /**
     * 从UDP网络传输,进行新增一条学生信息
     * 该方法返回0表示,接收数据成功,新增学生成功
     * 返回-1表示,网络传输过程中发生了异常
     * 返回-2,表示主动关闭停止网络连接
     * 返回-3表示,接收数据成功,但数组满了新增学生失败
     * 注:
     * 为了简化代码,该方式新增Student时就不做id,name等条件校验了
     * 失败只有一种可能,那就是数据源数组已满
     *
     * @return int
     * @author wuguidong@cskaoyan.onaliyun.com
    ...
    public int receiveDataFromNet() {
        return studentService.receiveDataFromNet();
    }

    /**
     * 关闭用于新增学生信息的UDP资源
     * 注意:
     * 此方法关闭的UDP资源和receiveDataFromNet方法使用的是同一资源
     *
     * @author wuguidong@cskaoyan.onaliyun.com
     * @since 11:25
     */
    public void closeUDPSocket(){
        studentService.closeUDPSocket();
    }
}

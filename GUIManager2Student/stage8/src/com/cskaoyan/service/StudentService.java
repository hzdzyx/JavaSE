...
public interface StudentService {
    ...
    /**
     * 利用TCP协议进行网络传输,实现初始化数据源数组
     * 在service层下实现一个TCP的服务端,接收客户端传输的Student数组对象
     * 然后依赖dao层,将接收到的Student数组赋值给数据源数组,完成初始化
     * 此方法返回:
     * 0,表示上述过程一切正常,完成初始化工作
     * -1,表示接收数据过程中发生了网络异常
     * -2,表示主动关闭停止网络连接
     *
     * @return int
    ...
    /**
     * 利用UDP协议进行网络传输,实现新增一条学生信息
     * 在service层下实现一个UDP的接收端,接收发送端发送的学生信息
     * 学生端发送学生信息是以字符串形式发送的,格式如下:
     * "stuId=1,name=李明,gender=男,school=xxx,major=软件工程,age=18,city=武汉,phone=13817618878,email=123@qq.com"
     * 作为接收端,需要解析这个字符串,并获取Student对象
     * 然后调用dao层,将Student对象添加到数据源数组中
     * 此方法返回:
     * 0,表示上述过程一切正常,接收数据成功且完成新增一条学生信息
     * -1,表示接收数据过程中发生了网络异常
     * -2,表示主动关闭停止网络连接
     * -3,接收数据成功,但数组满了新增学生失败
     *
     * @return int
     * @author wuguidong@cskaoyan.onaliyun.com
     * @since 17:44
     */
    int receiveDataFromNet();

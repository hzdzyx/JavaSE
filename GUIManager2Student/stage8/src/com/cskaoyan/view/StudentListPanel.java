...
public class StudentListPanel extends JPanel implements FocusListener {
    ...
    // 从网络接数据
    private JButton creatByNetBut = new JButton("从网络添加学生");
    ...
    public StudentListPanel(MainMenuFrame menuFrame, StudentController sc) {
    ...
    private void init() {
        ...
        // 从网络接收数据
        creatByNetBut.setFont(butFont);
        this.add(creatByNetBut, BorderLayout.CENTER);
        creatByNetBut.addActionListener(e -> {
            // 创建一个自定义按钮
            JButton overBut = new JButton("结束等待传输");
            overBut.setFont(new Font("Dialog", Font.BOLD, (int) (menuFrame.adaptiveWidth * 0.013)));

            JLabel label = new JLabel("正在等待网络传输数据...");
            label.setFont(new Font("Dialog", Font.BOLD, (int) (menuFrame.adaptiveWidth * 0.016)));
            ...
            JDialog dialog = new JDialog(menuFrame, "处理中");
            dialog.setModal(true);
            dialog.setContentPane(optionPane);
            dialog.setDefaultCloseOperation(JDialog.DO_NOTHING_ON_CLOSE);
            dialog.pack();
            dialog.setLocationRelativeTo(menuFrame);
            // 启动一个线程,用于监听网络传输,该线程会阻塞
            Thread netThread = new Thread(() -> {
                switch (studentController.receiveDataFromNet()) {
                    case 0:
                    ...
                    case -3:
                        // 数组满了
                        ShowWindowUtils.showWarning("数组已满,添加失败!");
                        break;
                }
                studentController.closeUDPSocket();
    ...
    private void refreshData() {
        menuFrame.setVisible(false);
        MainMenuFrame newMain = new MainMenuFrame(-2, this);
    }
